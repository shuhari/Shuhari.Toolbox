...
class RenamerConfig : public BaseConfig {
    Q_OBJECT
public:
    RenamerConfig(const QString& group, QObject* parent = nullptr);

    enum Type { FixedString = 1, Wildcard = 2, Regex = 3 };
    enum Target { File = 0x1, Dir = 0x2 };
    enum Options { Recursive = 0x1, DecodeUrl = 0x2 };
    ...
    inline int target()             { return _target->intValue(); }
    inline int options()            { return _options->intValue(); }
